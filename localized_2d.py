...
def main():
    ...
    '''
    apply tunneling operators and diagonalize
    '''
    e_diag, v_diag = H.diagonalize_tunneling_hamiltonian(wfn)

    with h5py.File(f'data_localized/data_B_{hamiltpar.B}_V0_{hamiltpar.V0}_tx_{hamiltpar.tx}_ty_{hamiltpar.ty}_{wfnpar.Mx}x{wfnpar.My}.h5', 'w') as f:
        ...
        f.create_dataset('e_tun', data=e_diag)
        f.create_dataset('v_tun', data=v_diag)
...
class Hamilt:
    ...
    def calc_Tx_Ty(self, wfn):
        Txpsi_ele = wfn.elewfn_shift(1, 0) + wfn.elewfn_shift(-1, 0)
        Typsi_ele = wfn.elewfn_shift(0, 1) + wfn.elewfn_shift(0, -1)

        Tx = np.einsum('ij,ij->', np.conjugate(wfn.elewfn), Txpsi_ele)
        Ty = np.einsum('ij,ij->', np.conjugate(wfn.elewfn), Typsi_ele)
        return Tx, Ty
    ...
    def calculate_Tx_Ty_loc_basis(self, wfn):
        B, tx, ty, V0, eta = self.hamilt_params.read()
        Mx, My, n, phi, k2 = self.wfn_params.read()
        ...
        Tx = (np.roll(OE, 1, axis=0) + np.roll(OE, -1, axis=0)) * OR
        Ty = (np.roll(OE, 1, axis=1) + np.roll(OE, -1, axis=1)) * OR
        ...
        B = OR * OE
        return Tx, Ty, Hr, Hi, B
        
    def tunneling_Tx_Ty_matrix(self, wfn):
        B, tx, ty, V0, eta = self.hamilt_params.read()
        Mx, My, n, phi, k2 = self.wfn_params.read()

        Txm, Tym, Hrm, Him, Bm = self.calculate_Tx_Ty_loc_basis(wfn)
